				...
				<td>
					<xsl:if test="@branchtype='folder'">
						...
						**  
						...
						<a class="branch">
						<xsl:if test="@code">  				
								<xsl:attribute name="onclick">toggle2(this, '<xsl:value-of select="@code"/>', '<xsl:value-of select="@alias"/>', '<xsl:value-of select="@lcid"/>', '<xsl:value-of select="@mapid"/>')</xsl:attribute>
						</xsl:if>
						<xsl:if test="not(@code)">
								<xsl:attribute name="onclick">toggle(this, '<xsl:value-of select="@alias"/>', '<xsl:value-of select="@lcid"/>')</xsl:attribute>
						</xsl:if>
							
						<!-- If the treeview is unfold, the image minus (-) is displayed-->
						<xsl:if test="@expanded">
							<xsl:if test="@expanded='true'">
									<img alt="" src="{$param-img-directory}minus.gif"/>
								</xsl:if>
								<!-- plus (+) otherwise-->
							<xsl:if test="@expanded='false'">
								<img alt="" src="{$param-img-directory}plus.gif"/>
								</xsl:if>
						</xsl:if>
					...
					<xsl:if test="@branchtype='leaf'">
						...
						**  
						...
						<a class="leaf">
							...
							<xsl:attribute name="onclick">MapInfoWebSelectedNodeChanged(this, '<xsl:call-template name="replace-string"><xsl:with-param name="text" select="@title"/><xsl:with-param name="from" select="$var-simple-quote"/><xsl:with-param name="to" select="$var-slash-quote"/></xsl:call-template>','<xsl:call-template name="replace-string"><xsl:with-param name="text" select="@code"/><xsl:with-param name="from" select="$var-simple-quote"/><xsl:with-param name="to" select="$var-slash-quote"/></xsl:call-template>', '<xsl:value-of select="@lcid"/>', '<xsl:value-of select="@mapid"/>', '<xsl:value-of select="@id"/>')</xsl:attribute>						
							<!-- if it is the last leaf, print a different image for the link to the folder-->
							<xsl:choose>
								<xsl:when test="position()=last()">
									<img alt="" src="{$param-img-directory}lastlink.gif"/>
								</xsl:when>
								<xsl:otherwise>
									<img alt="" src="{$param-img-directory}link.gif"/>
								</xsl:otherwise>
							</xsl:choose>
