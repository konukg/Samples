...
namespace DispNet
{
    public partial class pageTabFireAsrObl : UserControl
    {
        ...
        private void pageTabFireAsrObl_Load(object sender, EventArgs e)
        {
            ...
            l_iCatFireId = -1;
            ThreadStartFrmFireAsrObl();  // старт потока для вызова из данного потока формы с перечнем категорий пожара
        }
        ...
        private void CtgFireListLoad(bool blLoad)
        {
            ...
            FrmChsCatDrv.Instance.m_iSendMsgFrm = 2;    //категорию послать в эту форму
        ...
        private void LoadDataFire(bool blLoad)
        {
            ...
            c1CmbFireTypeObl.Columns[0].Caption = "Текст сообщения";
